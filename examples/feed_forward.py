...
image_transform = tv.transforms.Compose(
    [tv.transforms.ToTensor(), PoissonEncode(time_steps)]
)
...
hidden_neuron_weights = t.empty((hidden_neuron_count, input_neuron_count)).uniform_(
    0, 1
)  # 80x784
...
def calculate_lif_current(
    time: int,
    weights: t.Tensor,
    time_course_func: Callable[[t.Tensor], t.Tensor],
    last_fired_times: t.Tensor,
) -> t.Tensor:
    last_fired_time_step = last_fired_times[time]
    # Increment time by one to prevent weirdness when time is equal to zero.
    rescaled_time = time + 1
    time_course_input = last_fired_times.new_full(
        last_fired_time_step.shape, rescaled_time
    ) - (rescaled_time * last_fired_time_step)
...
for spike_trains, label in train_data_set:  # spike_train size: 784x10
    ...
    for i in range(time_steps):
        ...
        current = calculate_lif_current(
            i, hidden_neuron_weights, dirac_pulse, last_fired_times
        )
        ...
        spike_threshold = 65
        hidden_neuron_voltages = hidden_neuron_voltages.where(
            hidden_neuron_voltages < spike_threshold, reset_voltage
        )
